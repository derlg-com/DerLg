      ...
      {options.map((o) => (
        <div key={o.id} className="flex items-center justify-between border rounded-lg px-3 py-2">
          ...
          <div className="text-right">
            <p className="font-semibold text-sm">{formatCurrency(o.priceUsd, locale)}</p>
            <button onClick={() => onAction('select_transport', o.id, { optionId: o.id })} className="text-xs text-primary underline">{t('transport.select')}</button>
          </div>
        </div>
      ))}
    </div>
