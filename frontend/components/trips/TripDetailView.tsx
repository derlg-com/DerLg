...
export function TripDetailView({ id }: { id: string }) {
  ...
  // Only block the CTA when the payload explicitly marks the trip as not
  // bookable; a missing status (the common case) keeps the button active.
  const notBookable = ['inactive', 'sold_out', 'soldout', 'unavailable', 'archived'].includes(
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-4 pb-36">
      ...
      <div className="space-y-2">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 gap-2">
            ...
            <ShareButton title={trip.name} entity="trip" />
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-3 text-sm">
          ...
          {trip.maxGuests != null && trip.maxGuests > 0 ? (
            <span className="inline-flex items-center gap-1 text-muted-foreground">
              <Users className="h-4 w-4" aria-hidden />
              {t('detail.maxGuests', { n: trip.maxGuests })}
            </span>
          ) : null}
      ...
      <TripReviews subjectType="trip" subjectId={trip.id} />

      <RelatedTrips tripId={trip.id} />

      <BookNowCTA href={bookHref} priceUsd={trip.priceUsd} disabled={notBookable} />
    </div>
  )
}
